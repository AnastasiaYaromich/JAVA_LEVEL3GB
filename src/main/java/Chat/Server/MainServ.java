...
public class MainServ {
    ...
    public MainServ() {
        ...
        try {
            Handler fileHandler = new FileHandler("myLogChat.log", true);
...
//        AuthService.addUser("login12", "pass12", "nick12");
          ...
          //  System.out.println("Сервер запущен!");
            ...
            e.printStackTrace();
            logger.log(Level.WARNING, "Произошла ошибка при подключении клиента или при запуске сервера!");
        } finally {
            try {
                // Закрываем сокет
                socket.close();
                ...
                e.printStackTrace();
                logger.log(Level.WARNING, "Произошла ошибка при попытке закрыть сокет!");
            }
            try {
                // Закрываем сервер
                server.close();
                ...
                e.printStackTrace();
                logger.log(Level.WARNING, "Произошла ошибка при попытке закрыть сервер!");
            }
